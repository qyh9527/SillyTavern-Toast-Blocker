...
诊断数据由“本地性能诊断”开关控制采集，全部在“诊断概览”这一个可折叠面板内展示（v1.4.2 起与概览合并，独立数字区块已移除）。
...
### 14.1 概览折叠与长帧节流（v1.4.2）

概览默认折叠为一行标题，标题右侧保留健康徽标，折叠状态下也能一眼看到链路是否正常：

- 折叠头是原生 `<button>`，带 `aria-expanded` 与 `aria-controls`，键盘可达。
- 打开“本地性能诊断”开关时自动展开，关闭时自动收回。
- 用户手动点过折叠头后，本页内诊断开关不再覆盖用户的展开/收起选择。
- 主抽屉合上时跳过数字写回，此行为不变。

页面长帧（`observedLongFrames`）可能一次连续上百条：如果每条都触发一次界面刷新，诊断本身就会参与放大卡顿。v1.4.2 起计数即时入账，但通知界面的 `onStateChanged` 改为冷却节流——1 秒冷却窗口内的连续长帧只在窗口结束时触发一次刷新（renderer 的 `scheduleLongFrameNotify()`）。这与既有的 rAF 合批叠加，形成“每帧合批 + 每秒节流”两层限流。重绘批次、聚合等其他路径的刷新仍走 rAF 合批，不受此节流影响。
...
原区域新增 `diagnostics-view.ts` 提供的诊断概览，v1.4.2 起旧诊断数字也并入该区域并整体可折叠。概览显示规则存在性、方法守卫数量、宿主适配、当前通知、待显示队列、重绘批次和页面长帧。健康标识由规则与守卫等检查项决定，不能因为整页有长帧就判断插件故障。`mixed` 表示公开接口和兼容导入混用，是正常模式。
...
- 抽屉展开且概览展开时才写回诊断数字；打开抽屉时主动请求状态刷新。
- 页面长帧走额外的 1 秒冷却节流（见 14.1）。
...
| `diagnostics-view.test.mjs` | 实机数据语义、页面长帧不归因、空样本、故障提示与合并计数字段 |
...
11. 开启诊断后概览是否自动展开；关闭是否自动收回；手动折叠后开关是否不再覆盖选择。
12. 折叠的概览头部是否仍显示健康徽标；长帧风暴时数字是否约每秒刷新一次而非每条刷新。
