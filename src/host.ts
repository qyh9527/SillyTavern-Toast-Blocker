...
class ToastBlockerHost {
  ...
  async mountPanelWhenReady(): Promise<void> {
    ...
    wrapper.querySelector<HTMLButtonElement>('.qyh-toast-overview-toggle')?.addEventListener('click', event => {
      this.toggleOverview((event.currentTarget as HTMLButtonElement));
    });
  ...
  private isPanelContentVisible(): boolean {
  ...
  private isOverviewCollapsed(): boolean {
    const body = this.panel?.querySelector<HTMLElement>('#qyh-toast-overview-body');
    return Boolean(body?.hidden);
  }

  private setOverviewCollapsed(button: HTMLButtonElement | null, collapsed: boolean): void {
    const body = this.panel?.querySelector<HTMLElement>('#qyh-toast-overview-body');
  ...
  private syncOverviewCollapse(): void {
    if (this.overviewManuallyToggled) return;
    const button = this.panel?.querySelector<HTMLButtonElement>('.qyh-toast-overview-toggle') ?? null;
    const collapsed = !this.settings.diagnosticsEnabled;
    if (collapsed !== this.isOverviewCollapsed()) this.setOverviewCollapsed(button, collapsed);
  }
