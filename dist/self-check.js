...
export function createSelfCheckReport(status, source) {
    const redraw = status.redraw;
    const capabilities = {
    ...
    if (expectedRule && !status.earlyRuleInstalled)
        issues.push('早期规则缺失：请点击“修复早期规则”并重启前端。');
    if (Number(status.guardedMethods) < expectedGuards)
        issues.push('方法守卫数量不足：宿主或其他插件可能替换了 Toastr。');
